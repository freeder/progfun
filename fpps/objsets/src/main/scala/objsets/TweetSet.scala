...
abstract class TweetSet {
  ...
  def mostRetweetedAcc(accMost: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweeted = throw new java.util.NoSuchElementException("mostRetweeted of Empty")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = right.mostRetweetedAcc(left.mostRetweetedAcc(elem))

  def mostRetweetedAcc(accMost: Tweet): Tweet = {
    val most: Tweet = mostRetweeted
    if (most.retweets > accMost.retweets) most
    else accMost
  }
